...
### ARPU 역산 — 공시가 서로 맞는지 확인

```
33.92억엔 ÷ 평균 계약 스테이션 약 3,150 = 연 약 107.7만엔 = 월 약 89,700엔
```

중기계획의 2027년 목표 고객단가 **104,200엔/월**과 정합합니다 —
현재 약 89,700엔에서 목표까지 **약 +16%의 단가 인상 여력**이 계획에 잡혀 있다는 뜻입니다.
매출 성장이 스테이션 수 증가만이 아니라 **단가 상승**에서도 나오도록 설계돼 있습니다.
...
```mermaid
flowchart LR
    A["<b>성장률 1위 세그먼트</b><br/>방문간호<br/>약 +14~16% / 사업소 +9.9%"]
...
| 리스크 | 내용 | 심각도 |
|---|---|---|
| **규제 강화 (진행 중)** | サンウェルズ 부정청구 약 28.47억엔(2025.2 특별조사위) → アンビスHD 「医心館」 의혹(2026) → **厚労省 전국일제 현지조사(2026.1~, 방문간호 사상 최초)**. 2026년도 개정의 包括型訪問看護療養費·同一建物 구분은 호스피스형 주택 병설 모델을 정조준. 스테이션 폐업이 늘면 **eWeLL의 고객 모수가 직접 줄어듦** | **높음** |
...
- 일본 의료·개호에서 **가장 빠르게 성장하는 세그먼트**(방문간호, 약 +14~16%; 의료보험분만은 +20.4%)를 겨냥하고,
